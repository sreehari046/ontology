...
def main():
    print("🌿 Vrukshaayurveda AI Assistant (with ontology comments)")
    print("Ask me anything about plants, diseases, treatments, etc.")
    print("Type 'quit' to exit.\n")

    # Comprehensive system prompt with all example queries
...
"""
    ...
    while True:
        ...
        try:
            ...
            print(f"❌ Failed to generate SPARQL: {e}")
            continue
        ...
        # Step 3: Extract entity URIs from results to fetch comments
        ...
        if bindings:
            ...
            # Fetch comments for these URIs
            comments = fetch_comments(list(entity_uris))
        else:
            comments = {}

        # Step 4: Generate summary – STRICTLY FACTUAL
        print("📝 Generating summary...")
        if bindings:
            # Build a prompt with results and comments
...
Your answer:"""
            try:
                summary = ask_groq(summary_prompt, temperature=0.0, max_tokens=300)  # Temperature 0 for deterministic output
                print(f"💬 Summary: {summary}\n")
            except Exception as e:
                # Fallback: simple list
                items = []
                for b in bindings:
                    # Try to get a readable name from common variables
                    for key in ["name", "plantName", "materialName", "diseaseName", "symptomName", "nakshatraName", "seasonName", "explanation"]:
                        if key in b and b[key]["type"] == "literal":
                            items.append(b[key]["value"])
                            break
                    else:
                        # fallback to URI last part
                        for key in ["plant", "material", "disease", "symptom", "nakshatra", "soil", "season", "technique", "practice"]:
                            if key in b:
                                uri = b[key]["value"]
                                items.append(uri.split('/')[-1])
                                break
                if items:
                    summary = "Found: " + ", ".join(items) + "."
                else:
                    summary = "Results were found but no readable names."
                print(f"💬 Summary: {summary}\n")
        else:
            # No results – suggest alternative queries based on ontology structure
...
Your response:"""
            try:
                summary = ask_groq(no_results_prompt, temperature=0.2, max_tokens=200)
                print(f"💬 Summary: {summary}\n")
            except Exception as e:
                print("💬 Summary: No information found in the ontology.\n")
